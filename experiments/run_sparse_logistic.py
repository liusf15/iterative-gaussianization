...
def get_reference_samples():
    return pd.read_csv("experiments/german_reference_samples_unc.csv", index_col=0).values
...
def fit_NSF(logp_fn, seed=0, nsample=1000, ntrain=1000, n_layer=4, beta_0=0.1, learning_rate=0.1, num_bins=10, range_max=7., max_iter=100):
    model_nf = NeuralSplineFlow(dim=d, n_layers=n_layer, hidden_dims=[4], num_bins=num_bins, range_min=-range_max, range_max=range_max, boundary_slopes='unconstrained')
...
def evaluate(target, mcmc_samples_unc, flow_samples, log_q):
    ...
    metrics['moment1'] = jnp.mean(samples_constrain, 0)
    metrics['moment2'] = jnp.mean(samples_constrain**2, 0)

    mcmc_samples_constrain = target.param_constrain(mcmc_samples_unc)
    metrics['ref_moment1'] = jnp.mean(mcmc_samples_constrain, 0)
    metrics['ref_moment2'] = jnp.mean(mcmc_samples_constrain**2, 0)
    return metrics
...
def main(args):
    ...
    if args.algo == 'gaussianization':
        ...
        start = time.time()
        transformed_samples, logq = fit_NSF(logp_fn_shifted, args.seed, args.nsample, args.ntrain, args.n_layer, args.beta_0, args.learning_rate, args.num_bins, args.range_max, args.max_iter)
        time_elapsed = time.time() - start
    else:
